...
const updateVerified = (warshipId, verified) => {
    warshipAPI.updateVerified(warshipId, verified);
};
            ...
            <div class="card">
                ...
                >
                    ...
                    <Column field="verified" header="是否通过审核" dataType="boolean" bodyClass="text-center" style="min-width: 8rem">
                        <template #body="{ data }">
                            <i class="pi" @click="data.verified = !data.verified; updateVerified(data.id, data.verified)" :class="{ 'text-green-500 pi-check-circle': data.verified, 'text-pink-500 pi-times-circle': !data.verified }"></i>
                        </template>
                        <template #filter="{ filterModel }">
